    ...
    for chain in model:
        for res in chain:
            ...
            if rn in PROTEIN_RESNAMES or rn in SOLVENT:
                continue
            heavy = [a for a in res if norm_elem(a.element) != "H"]
...
def extract_pocket_from_pdb(pdb_path, ligand_resname="AUTO",
    ...
    for atom in lig_res:
        if atom.element and atom.element.strip() not in ("", "X"):
            el = norm_elem(atom.element)
        else:
            nm = atom.get_name().strip()
            el = norm_elem(nm[:2] if nm[:2].upper() in ("CL", "BR") else nm[:1])
        try:
        ...
        for res in chain:
            ...
            if rn in ION_CHARGES:
                ...
                extra.append(dict(chain=chain.get_id().strip(), resid=res.get_id()[1], icode="",
                                  resname=rn, name=a.get_name(), elem=norm_elem(a.element),
                                  xyz=np.array(a.get_coord(), float), q=float(ION_CHARGES[rn]),
                                  sigma=0.0, eps=0.0))
            elif rn in WATER_RESNAMES and include_waters:
                at = list(res)
                if not any(norm_elem(a.element) == "H" for a in at):
                    n_w_skip += 1; continue
                for a in at:
                    el = norm_elem(a.element)
                    extra.append(dict(chain=chain.get_id().strip(), resid=res.get_id()[1], icode="",
...
def save_report(pdb_path, pocket, desc, mep, qmmm,
    ...
    data = {
        ...
        "Hirshfeld_charges_gas"     : [round(float(x),4) for x in fukui.get("q_gas", [])],
